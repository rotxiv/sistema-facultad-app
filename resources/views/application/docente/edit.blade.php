        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                ...
                <div class="p-4 md:p-5 max-h-[70vh] overflow-y-auto">
                    <form class="space-y-4" action="{{ route('docentes.update', $docente->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <input type="text" name="carnet" id="carnet"
                            value="{{ old('carnet', $docente->persona->carnet ?? '') }}"
                        ...
                        <input type="text" name="nombre" id="nombre"
                            value="{{ old('nombre', $docente->persona->nombre ?? '') }}"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg 
                                focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 
                                dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                            required />
                        ...
                        <select name="sexo" id="sexo"
                            ...
                            <option value="M" {{ old('sexo', $docente->persona->sexo ?? '') == 'M' ? 'selected' : '' }}>M - Masculino</option>
                            <option value="F" {{ old('sexo', $docente->persona->sexo ?? '') == 'F' ? 'selected' : '' }}>F - Femenino</option>
                        </select>
