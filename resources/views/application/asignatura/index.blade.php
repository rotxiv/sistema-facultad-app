                ...
                <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5 max-h-[70vh] overflow-y-auto">
                        <form class="space-y-4" action="{{ route('asignaturas.store') }}" method="POST">
                            @csrf
                            <div>
                                <label for="sigla" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Sigla de la Asignatura</label>
                                <input type="text" name="sigla" id="sigla" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required />
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                ...
                <tr id="row-{{ $asignatura->id }}" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                    ...
                    <!-- Sigla (modo visual y editable) -->
                    <td class="px-6 py-4">
                        <span class="descripcion-text">{{ $asignatura->sigla }}</span>
                        <input type="text" name="sigla" value="{{ $asignatura->sigla }}"
                            class="form-editar hidden descripcion-input w-full border rounded px-2 py-1 text-gray-900 dark:bg-gray-600 dark:text-white">
                    </td>

                    <!-- Descripción (modo visual y editable) -->
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4 text-center">
                        ...
                        <form method="POST" action="{{ route('asignaturas.update', $asignatura->id) }}" class="inline-block">
                            @csrf
                            @method('PUT')
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.editar-btn').forEach(boton => {
                boton.addEventListener('click', function () {
                    ...
                    // Ocultar todos los spans de texto
                    fila.querySelectorAll('.descripcion-text').forEach(span => {
                        span.classList.add('hidden');
                    });

                    // Mostrar todos los inputs de edición
                    fila.querySelectorAll('.descripcion-input').forEach(input => {
            ...
            document.querySelectorAll('.cancelar-btn').forEach(boton => {
                boton.addEventListener('click', function() {
                    ...
                    // Mostrar todos los spans de texto
                    fila.querySelectorAll('.descripcion-text').forEach(span => {
