<x-layouts.app :title="__('Lista de Aulas')">
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg p-4 space-y-4">
        ...
        <div class="flex flex-wrap gap-4 mb-4">
            ...
            <form action="{{ route('roles.deleted-index') }}" method="GET">
                @csrf
                ...
                <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5 max-h-[70vh] overflow-y-auto">
                        <form class="space-y-4" action="{{ route('roles.store') }}" method="POST">
                            @csrf
                            <div>
                                <label for="nombre" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nombre del Rol</label>
                                ...
                                <label for="descripcion" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Descricion del Rol</label>
                                ...
                                <button type="submit" class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Añadir Rol</button>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            <caption class="p-5 text-lg font-semibold text-left rtl:text-right text-gray-900 bg-white dark:text-white dark:bg-gray-800">
                ...
                <p class="mt-1 text-sm font-normal text-gray-500 dark:text-gray-400">
                    Todos los roles que están disponibles actualmente.
                </p>
            </caption>
            ...
            <tbody>
                ...
                <tr id="row-{{ $rol->id }}" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    <form method="POST" action="{{ route('roles.update', $rol->id) }}" class="w-full flex items-center">
                        @csrf
                        @method('PUT')

                        <!-- ID -->
                        <td class="px-4 py-3 align-middle">{{ $rol->id }}</td>

                        <!-- Nombre -->
                        <td class="px-4 py-3 align-middle">
                            <span class="nombre-text">{{ $rol->nombre }}</span>
                            <input type="text" name="nombre" value="{{ $rol->nombre }}"
                                class="nombre-input hidden w-full border rounded px-2 py-1 text-gray-900 dark:bg-gray-600 dark:text-white">
                        </td>
                        ...
                        <td class="px-4 py-3 align-middle">
                            ...
                            <input type="text" name="descripcion" value="{{ $rol->descripcion }}"
                                class="descripcion-input hidden w-full border rounded px-2 py-1 text-gray-900 dark:bg-gray-600 dark:text-white">
                        </td>
                    ...
                    <!-- Formulario de eliminación fuera del form de edición -->
                    <td class="px-4 py-3 align-middle">
                        <form method="POST" action="{{ route('roles.destroy', $rol->id) }}" class="inline-block ml-2" onsubmit="return confirm('¿Estás seguro de eliminar este rol?');">
                            @csrf
                            @method('DELETE')
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.editar-btn').forEach(boton => {
                boton.addEventListener('click', function() {
                    ...
                    fila.querySelector('.nombre-text').classList.add('hidden');
                    fila.querySelector('.descripcion-text').classList.add('hidden');
                    fila.querySelector('.nombre-input').classList.remove('hidden');
                    fila.querySelector('.descripcion-input').classList.remove('hidden');
            ...
            document.querySelectorAll('.cancelar-btn').forEach(boton => {
                boton.addEventListener('click', function() {
                    const fila = this.closest('tr');
                    fila.querySelector('.nombre-input').classList.add('hidden');
                    fila.querySelector('.descripcion-input').classList.add('hidden');
                    fila.querySelector('.nombre-text').classList.remove('hidden');
